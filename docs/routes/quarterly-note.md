...
These routes deal with reading, writing and updating quarterly notes. Their URLs start with `obsidian://actions-uri/quarterly-note/…`.

All routes listed here will test whether Quarterly Note functionality is enabled in Obsidian and if not, will return an `x-error` callback.  (Quarterly Notes are a feature of [@liamcain](https://github.com/liamcain)'s community plugin [Periodic Notes](https://github.com/liamcain/obsidian-periodic-notes).)  The settings made in the plugin's UI are honored, i.e. date format, the set folder for quarterly notes etc. are taken into account when fetching, creating and updating notes.

<div id="toc"></div>


## DEPRECATION NOTICE
<span class="tag tag-version">v1.6</span> All dedicated periodic note-related routes (`/daily-note/*`, `/weekly-note/*`, `/monthly-note/*`, `/quarterly-note/*`, `/yearly-note/*`) are officially deprecated, and will be removed in early 2025. Their functionality is now part of the [`/note` routes](/obsidian-actions-uri/routes/note/). Please update your scripts accordingly.
...
## `/quarterly-note/list`
Returns a list of all quarterly notes.
...
## `/quarterly-note/open-current`
Opens the current quarterly note in Obsidian.
...
## `/quarterly-note/open-most-recent`
Opens the most recent quarterly note in Obsidian.  If there is a current quarterly note (i.e. one for today), that's considered the most recent one, otherwise the most recent *past* quarterly note is returned.
...
Creates a new quarterly note. If a current quarterly note is already present, an error will be returned.
...
## `/quarterly-note/append`
Appends text to today's quarterly note, either to the very end of the note (default) or to the section below a particular headline.
...
## `/quarterly-note/search-string-and-replace`
Does text replacement in today's quarterly note.  The search term is used as-is, i.e. it's a string search.
...
## `/quarterly-note/search-regex-and-replace`
Does text replacement in today's quarterly note.  The search term is used as a pattern, i.e. it's a regular expression search.
